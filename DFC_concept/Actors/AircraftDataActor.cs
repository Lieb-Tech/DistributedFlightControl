...
{
    class AircraftDataActor : ReceiveActor
    {
        ...
        public AircraftDataActor()
        {
            Receive<AircraftRequest>(r =>
            {
                if (aircraft.ContainsKey(r.Craft.ToUpper()))
            ...
            Receive<DataRequest>(r =>
            {
                if (datas.ContainsKey(r.Hex.ToUpper()))
                    Sender.Tell(new DataResponse()
                    {
